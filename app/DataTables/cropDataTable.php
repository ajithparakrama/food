    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addIndexColumn()
            ->addColumn('action', function($item){
                $btn = '';
                $btn .= '<a href="'.route('crop.edit',$item->id).'" class="btn btn-xs btn-info" > <i class="fa fa-pen"></i></a>';

                $btn .= '<form  action="' . route('crop.destroy', $item->id) . '" method="POST" class="d-inline" >
           ' . csrf_field() . '
               ' . method_field("DELETE") . '
